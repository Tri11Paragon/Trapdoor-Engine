...
<head>
    <meta charset="utf-8"/>
	<title>Code Viewer. Source code: src/5.advanced_lighting/4.normal_mapping/normal_mapping.cpp</title>
...
int main()
{
   ...
   <function id='60'> glEnable(</function>GL_DEPTH_TEST);
    ...
    // -------------------------
    Shader shader("<a href='code_viewer_gh.php?code=src/5.advanced_lighting/4.normal_mapping/4.normal_mapping.vs' target='_blank'>4.normal_mapping.vs</a>", "<a href='code_viewer_gh.php?code=src/5.advanced_lighting/4.normal_mapping/4.normal_mapping.fs' target='_blank'>4.normal_mapping.fs</a>");
    ...
    while (<function id='14'>!glfwWindowShouldClose(</function>window))
    {
       ...
       <function id='10'> glClear(</function>GL_COLOR_BUFFER_BIT | GL_DEPTH_BUFFER_BIT);
        ...
        model =<function id='57'> glm::rotate(</function>model,<function id='63'> glm::radians(</function>(float<function id='47'>)glfwGetTime(</function>) * -10.0f), glm::normalize(glm::vec3(1.0, 0.0, 1.0))); // rotate the quad to show normal mapping from multiple directions
