...
<head>
    <meta charset="utf-8"/>
	<title>Code Viewer. Source code: src/3.model_loading/1.model_loading/model_loading.cpp</title>
...
int main()
{
   ...
   <function id='60'> glEnable(</function>GL_DEPTH_TEST);
    ...
    // -------------------------
    Shader ourShader("<a href='code_viewer_gh.php?code=src/3.model_loading/1.model_loading/1.model_loading.vs' target='_blank'>1.model_loading.vs</a>", "<a href='code_viewer_gh.php?code=src/3.model_loading/1.model_loading/1.model_loading.fs' target='_blank'>1.model_loading.fs</a>");
    ...
    while (<function id='14'>!glfwWindowShouldClose(</function>window))
    {
       ...
       <function id='10'> glClear(</function>GL_COLOR_BUFFER_BIT | GL_DEPTH_BUFFER_BIT);
        ...
        model =<function id='55'> glm::translate(</function>model, glm::vec3(0.0f, 0.0f, 0.0f)); // translate it down so it's at the center of the scene
        model =<function id='56'> glm::scale(</function>model, glm::vec3(1.0f, 1.0f, 1.0f));	// it's a bit too big for our scene, so scale it down
