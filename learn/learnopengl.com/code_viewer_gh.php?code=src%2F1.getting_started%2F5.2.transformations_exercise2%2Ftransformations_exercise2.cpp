...
<head>
    <meta charset="utf-8"/>
	<title>Code Viewer. Source code: src/1.getting_started/5.2.transformations_exercise2/transformations_exercise2.cpp</title>
...
int main()
{
   ...
   <function id='19'> glfwMakeContextCurrent(</function>window);
    ...
    // ------------------------------------
    Shader ourShader("<a href='code_viewer_gh.php?code=src/1.getting_started/5.2.transformations_exercise2/5.2.transform.vs' target='_blank'>5.2.transform.vs</a>", "<a href='code_viewer_gh.php?code=src/1.getting_started/5.2.transformations_exercise2/5.2.transform.fs' target='_blank'>5.2.transform.fs</a>");
   ...
   <function id='15'> glTexParameteri</function>(GL_TEXTURE_2D, GL_TEXTURE_MAG_FILTER, GL_LINEAR);
    ...
    while (<function id='14'>!glfwWindowShouldClose(</function>window))
    {
       ...
       <function id='48'> glBindTexture(</function>GL_TEXTURE_2D, texture2);
        ...
        // first container
        // ---------------
        transform =<function id='55'> glm::translate(</function>transform, glm::vec3(0.5f, -0.5f, 0.0f));
        transform =<function id='57'> glm::rotate(</function>transform, (float<function id='47'>)glfwGetTime(</function>), glm::vec3(0.0f, 0.0f, 1.0f));
        // get their uniform location and set matrix (using glm::value_ptr)
        unsigned int transformLoc =<function id='45'> glGetUniformLocation(</function>ourShader.ID, &quot;transform&quot;);
       <function id='44'> glUniformM</function>atrix4fv(transformLoc, 1, GL_FALSE, glm::value_ptr(transform));
       ...
       <function id='2'> glDrawElements(</function>GL_TRIANGLES, 6, GL_UNSIGNED_INT, 0);
        ...
        transform =<function id='55'> glm::translate(</function>transform, glm::vec3(-0.5f, 0.5f, 0.0f));
        float scaleAmount = sin<function id='47'>(glfwGetTime(</function>));
        transform =<function id='56'> glm::scale(</function>transform, glm::vec3(scaleAmount, scaleAmount, scaleAmount));
       <function id='44'> glUniformM</function>atrix4fv(transformLoc, 1, GL_FALSE, &amp;transform[0][0]); // this time take the matrix value array's first element as its memory pointer value
