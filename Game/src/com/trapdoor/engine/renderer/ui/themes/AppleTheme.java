...
public class AppleTheme implements ITheme {
	...
	public ImGuiStyle applyStyle(ImGuiStyle style) {
        ...
        style.setColor(ScrollbarGrabActive]   = ImVec4(0.49f, 0.49f, 0.49f, 1.00f);
        style.setColor(ComboBg]               = ImVec4(0.86f, 0.86f, 0.86f, 0.99f);
        ...
        if( darkStyle )
        {
            for (int i = 0; i <= COUNT; i++)
            {
                ...
                float H, S, V;
                ImGui.colorConvertRGBtoHSV( new float[] {col.x, col.y, col.z}, new float[] {H, S, V} );

                if( S < 0.1f )
                {
                   V = 1.0f - V;
                }
                ImGui.colorConvertHSVtoRGB( new float[] {H, S, V}, new float[] {col.x, col.y, col.z} );
                if( col.w < 1.00f )
                {
                    col.w *= alpha;
                }
