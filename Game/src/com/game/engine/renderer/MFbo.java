...
public class MFbo {
	 ...
	 *            this FBO should use.
	 */
	public MFbo(int width, int height, int depthBufferType) {
		this.width = width;
		this.height = height;
	 ...
	 */
	public MFbo(int width, int height) {
		this.width = width;
		this.height = height;
	...
	public void resolveToFBO(int readBuffer, MFbo outputFBO) {
	...
	private int createMultisampleColorAttachment(int attachemnt) {
		...
		GL30.glRenderbufferStorageMultisample(GL30.GL_RENDERBUFFER, 4, GL11.GL_RGBA8, width, height); // HAS MULTISAMPLE AMOUNT
